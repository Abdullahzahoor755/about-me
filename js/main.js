...
class MusicController {
    constructor() {
        ...
        this.overlay = document.getElementById('music-overlay');
        this.icon = this.toggleBtn?.querySelector('i');
        this.tooltip = this.toggleBtn?.querySelector('.music-tooltip');
    ...
    init() {
        ...
        // Show overlay for initial interaction
        if (this.overlay) {
            const startMusicHandler = (e) => {
            ...
            this.overlay.addEventListener('click', startMusicHandler);
    ...
    handleScroll() {
        ...
        const scrollProgress = Math.min(currentScrollY / (documentHeight - windowHeight), 1);

        // Determine scroll direction
        const isScrollingDown = scrollDelta > 0;

        // Calculate target volume based on scroll position
        let targetVolume = this.baseVolume + (scrollProgress * (this.maxVolume - this.baseVolume));

        // Adjust based on scroll direction
        if (isScrollingDown) {
            // Increase faster when scrolling down
            targetVolume += 0.1;
        } else if (scrollDelta < 0) {
            // Decrease when scrolling up
            targetVolume -= 0.05;
        }

        // Clamp volume
        targetVolume = Math.max(this.minVolume, Math.min(this.maxVolume, targetVolume));

        // Smooth transition
        this.currentVolume += (targetVolume - this.currentVolume) * 0.1;
        this.audio.volume = this.currentVolume;
    ...
    updateButtonState() {
        if (!this.icon || !this.toggleBtn) return;

        if (this.isPlaying) {
            this.icon.className = 'fas fa-volume-up';
            this.toggleBtn.classList.add('active');
            if (this.tooltip) {
                this.tooltip.textContent = 'Click to mute';
            }
        } else {
            this.icon.className = 'fas fa-volume-mute';
            this.toggleBtn.classList.remove('active');
            if (this.tooltip) {
                this.tooltip.textContent = 'Click to unmute';
            }
        }
    }
}
